...
const eventsGet = async (req, res, next) => {
    const events = await Events.find().populate("users").populate("artists").populate("halls");
    res.json(events);
}
...
const eventGetById = async(req, res, next) => {
    try {
        ...
        // console.log(id)
        const event = await Events.findById(eventName).populate("users").populate("artists").populate("halls");
        // 
        res.json(event)
